...
// Only these system-level roles should appear in the SuperAdmin assign page
const SYSTEM_ROLE_NAMES = ['superadmin', 'orgsadmin']

export default function HQAssignUserRoles() {
  ...
  const { data: allRoles = [], isFetching: rolesLoading } = useQuery<RoleListItem[]>({
    queryKey: ['hq-roles-all'],
    queryFn: getRoles,
    staleTime: 60_000,
  })

  // Filter to only system-level roles
  const roles = useMemo(() =>
    allRoles.filter(r => SYSTEM_ROLE_NAMES.includes(r.name.toLowerCase())),
    [allRoles]
  )

  const { data: allResources = [], isFetching: permsLoading } = useQuery<PermissionResource[]>({
    queryKey: ['hq-permissions-all'],
    queryFn: AdmingetPermissionsAll,
    staleTime: 60_000,
  })

  const resources = useMemo(() =>
    allResources.filter(r => (r.permissions || []).length > 0),
    [allResources]
  )
  ...
  const {
    ...
    queryKey: ['hq-role-details', selectedRoleId],
    queryFn: () => getRoleHQDetails(selectedRoleId as string | number),
    ...
    <div className="grid gap-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-gradient-to-br from-purple-500/20 to-indigo-500/20 rounded-xl ring-1 ring-purple-500/20">
            <Shield className="w-5 h-5 text-purple-400" />
          </div>
          <div>
            <h1 className="text-xl font-semibold text-foreground">HQ Admin - Assign Role Permissions</h1>
            <p className="text-sm text-foreground/40">
              Assign permissions to roles across all organizations &middot; {resources.length} resource groups &middot; {totalPermissions} permissions
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="flex-1 min-w-0">
              ...
              <select
                className="w-full bg-foreground/[0.04] border border-foreground/10 rounded-xl px-4 py-3 text-sm text-foreground
                           outline-none focus:border-purple-500/40 focus:ring-1 focus:ring-purple-500/20 transition-all
              ...
              <div className="flex items-center gap-3 lg:gap-4">
                  ...
                  <svg className="w-14 h-14 -rotate-90" viewBox="0 0 56 56">
                    ...
                    <circle cx="28" cy="28" r="24" fill="none" strokeWidth="3"
                      ...
                      className={clsx(
                        ...
                        pctGlobal === 100 ? 'text-emerald-400' : pctGlobal > 0 ? 'text-purple-400' : 'text-foreground/20'
                      )}
                      stroke="currentColor"
                    />
                  </svg>
                ...
                <button
                  className={clsx(
                    ...
                    'hover:from-purple-500 hover:to-indigo-400 hover:shadow-purple-500/30',
          ...
          <div className="h-1 bg-foreground/[0.04]">
            <div
              className={clsx(
                ...
                pctGlobal === 100 ? 'bg-emerald-500' : pctGlobal > 0 ? 'bg-purple-500' : ''
              )}
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-foreground/[0.04] border border-foreground/[0.08] rounded-xl pl-9 pr-9 py-2.5 text-sm
                         ...
                         focus:border-purple-500/30 focus:ring-1 focus:ring-purple-500/15 transition-all"
              value={search}
                ...
                <div className="h-[2px] bg-foreground/[0.04]">
                  <div
                    className={clsx(
                      ...
                      all ? 'bg-emerald-500/70' : some ? 'bg-purple-500/60' : ''
                    )}
                    style={{ width: `${pct}%` }}
                  />
                </div>
                      ...
                      return (
                        ...
                        >
                          <input
                            type="checkbox"
                            className={clsx(
                              'mt-[3px] w-3.5 h-3.5 rounded accent-purple-500 cursor-pointer shrink-0 transition-opacity',
      ...
      {!isLoading && resources.length === 0 && (
        <div className="rounded-2xl bg-surface-elevated border border-foreground/[0.06] p-12 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm text-foreground/40">No permissions available</p>
        </div>
      )}
      ...
      {!rolesLoading && roles.length === 0 && (
        <div className="rounded-2xl bg-surface-elevated border border-foreground/[0.06] p-12 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm text-foreground/40">No system roles available</p>
        </div>
      )}
    </div>
  )
